...
public class Geometry : MonoBehaviour {
	...
	public Vector2 slices = Vector2.one;
	...
	public void Generate () {
		...
		for (int index = 0; index < positions.Length; ++index) {
			positions[index] = new Vector3(Random.Range(-1f,1f), Random.Range(-1f,1f), Random.Range(-1f,1f));
			colors[index] = new Color(Random.Range(-1f,1f), Random.Range(-1f,1f), Random.Range(-1f,1f), Random.Range(-1f,1f));
		}
		gameObject.GetComponent<MeshFilter>().mesh = Geometry.Particles(positions, colors, null, slices.x, slices.y);
		gameObject.GetComponent<MeshFilter>().sharedMesh.bounds = new Bounds(Vector3.zero, Vector3.one * 100f);
	}
	...
	static public Mesh Grid (int width, int height, bool wireframe = false) {
		...
		for (int i = 0, y = 0; y <= height; y++) {
			for (int x = 0; x <= width; x++, i++) {
				...
				vertices[i] = new Vector3((xx*2f-1f)*10f, (yy*2f-1f)*10f, 10f);
